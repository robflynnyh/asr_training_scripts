...
def validate_one_epoch(args, model, val_dataloader, device, sanity_check=False):
    ...
    for batch in pbar:
        ...
        for ix, sub_batch_ in enumerate(batch):
            ...
            eos_id = -100 if not args.bos_eos else 0 # don't use eos
    ...
    for epoch_ in range(args.epochs): # todo move epoch loop to model utils as is consistent across model types
        ...
        with ema.average_parameters(): # evaluate using ema of the parameters
            vloss = validate_one_epoch(
                args,
                model, 
                Sampler(val_samples, batch_size=args.batch_size, tokenizer=tokenizer, shuffle=False, split_into_splits=15), 
                device, 
                sanity_check=False
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data', type=str, default='/store/store1/data/spotify_podcasts_textonly/1t3_spotify_podcasts_train.csv')
    parser.add_argument('--dev_data', type=str, default='/store/store1/data/spotify_podcasts_textonly/1t3_spotify_podcasts_dev.csv')
