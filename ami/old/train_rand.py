...
def train_one_epoch(model, optim, schedular, train_dataloader, device, scaler=None, ema=None):
    ...
    for i, batch in enumerate(pbar):
        ...
        scaler.scale(loss_a).backward() if scaler is not None else loss_a.backward()
        
        if (i + 1) % args.accumulate_gradients == 0: 
            scaler.step(optim) if scaler is not None else optim.step()
            scaler.update() if scaler is not None else None
            optim.zero_grad() 
...
def main(args):
    ...
    train_dataloader = iid_dataloader.get_data_loader(ami_dict['train'], tokenizer=tokenizer, shuffle=True, max_sample_size=33)
    dev_dataloader = iid_dataloader.get_data_loader(ami_dict['dev'], tokenizer=tokenizer, shuffle=False, max_sample_size=33)

    if args.run_test == True:
        test_dataloader = iid_dataloader.get_data_loader(ami_dict['test'], tokenizer=tokenizer, shuffle=False, max_sample_size=14)
    ...
    # print non-shared parameters
    ttl = 0
    for name, param in model.named_parameters():
        if param.requires_grad:
            ttl += param.numel()
    print(f'\nTotal number of parameters: {ttl/1e6}M\n')
