...
from model_utils import load_checkpoint, load_nemo_checkpoint, load_transducer_model, write_to_log, \
...
def validate_one_epoch(model, val_dataloader, device, sanity_check=False):
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            encoded, encoded_len = model.forward(input_signal=input_signal, input_signal_length=input_signal_lengths)
            decoder, target_length, states = model.decoder(targets=targets, target_length=target_lengths)
...
def main(args):
    model = load_transducer_model(args)
    ...
    train_dataloader = tools.load_dataloader(ami_dict['train'], tokenizer, 350, True)
    dev_dataloader = tools.load_dataloader(ami_dict['dev'], tokenizer, 350, False)
    ...
    wandb.init(project="deliberation-Custom_ami")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_lr', type=float, default=7e-3)
    parser.add_argument('--step_size', type=int, default=400)
    parser.add_argument('--schedular_data', type=str, default='./schedular_data_transducer.json')
