...
class DepthWiseSeparableConv1D(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, stride = 1):
        super().__init__()
        self.pointwise = nn.Conv1d(dim_in, dim_out, kernel_size = 1, padding = 0)
        self.act = nn.SiLU()
        self.depthwise = nn.Conv1d(dim_out, dim_out, kernel_size = kernel_size, padding = 'same', groups = dim_out, stride = stride)
...
class transformer(nn.Module):
    def __init__(
        ...
        token_state_sizes = [28*(i+1) for i in range(depth)] # quantize outputs and shift them for each layer as targets
        ...
        for i in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                Chainer( # ??
                    PreNorm(dim, DepthWiseSeparableConv1D(dim_in = dim, dim_out = dim, kernel_size = 3, stride = 2)),
                    PreNorm(dim, nn.Linear(dim, token_state_sizes[i])),
                    VectorQuantize(
                        dim = dim,
                        codebook_dim = 8,
                        codebook_size = token_state_sizes[i],
                        use_cosine_sim = True,
                        commitment_weight = 0,
                    )
                ),
...
class transformer_lm(nn.Module):
    ...
    def self_condition_fn(self):
        def self_condition(x, logit_layer):
    ...
    def forward(self, x, mask=None):
        x = self.embedding(x)
        x = self.abs_pos(x)
        logits = self.layers(x, mask=~mask if mask is not None else None, self_condtioning=self.self_condition_fn())
