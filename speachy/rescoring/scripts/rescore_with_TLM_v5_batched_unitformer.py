...
@torch.no_grad()
def get_text_batched_probability(args, model, tokenizer, batched_text, cached_states=None, layer_weighting=List[float]):
    ...
    logprobs = outputs['token_losses']
    logprobs = logprobs.sum(0)
    cached_states = None
        ...
        for idx in utt['beams'][0].keys():
            ...
            if len(batch_stack) >= args.batch_size or idx == len(utt['beams'][0]) - 1 or idx == max_beam - 1:
                ...
                for ix, b_ix in enumerate(batch_stack):
                    if not args.use_targets and ix == 0 and exists(cache):
                        kvs_to_cache = {
...
def run_random_search(args, model, tokenizer, hypothesis):
    ...
    tlm_scales = [0.0, 2.0]
    ngram_scales = [0.3, 1.25] if not args.only_TLM else [0.0, 0.0]
    length_penalties = [0.0, 0.0] #not used anymore
    bpe_length_penalty_weights = [-1.0, 3.0]'''
    print(f'Running Random search')
   
    scores_v_params_v_hyps = []
    lowest_wer = 1.0
    ...
    try:
        while (time.time() - start_time) < args.random_search_time:
            ...
            scores_v_params_v_hyps.append((score, hyperparameters, hyp))
            if score < lowest_wer:
                lowest_wer = score
    ...
    scores_v_params_v_hyps = sorted(scores_v_params_v_hyps, key=lambda x: x[0])
    print(f'Best params: {scores_v_params_v_hyps[0][1]}')
    print(f'Best wer: {scores_v_params_v_hyps[0][0]}')

    hypothesis = scores_v_params_v_hyps[0][2]

    if args.eval_with_sclite != '':
        print(f'Evaluating with sclite')
...
'''
Best params: {'tlm_mean': tensor(-19.4433), 'tlm_std': tensor(17.6366), 'bpe_lm_weight': tensor(1.0319), 'tlm_scale': tensor(0.8861), 'ngram_scale': tensor(0.4397), 'bpe_length_penalty_weight': tensor(2.4690)}
Best wer: 0.21165097406001634
'''

def main(args, hypothesis_dev, hypothesis_test):
    ...
    if hypothesis_dev.__class__.__name__ == 'list': # only a list if it hasn't been processed yet   
    ...
    if args.eval_with_sclite != '':
        ...
        refname, hypname = write_trn_files(
            ...
            speakers = speakers,
            encoded_lens = utt_durations,
            fname = args.sclite_name if args.sclite_name == 'date' else f'test_{args.sclite_name}',
            out_dir = args.eval_with_sclite
        )
        wer = eval_with_sclite(ref=refname, hyp=hypname, SCLITE_PATH=sclite_path)
        print(f'WER (sclite): {wer}')
...
# 200, 110 ?
#Lowest WER: 0.09733874104116895 with params: bpe_lm_weight: 0.5821194774690709, tlm_scale: 4.273038081006296, ngram_scale: 1.1849170328779142, bpe_length_penalty_weight: 2.848608020649601
#./checkpoints/unitformer_test/checkpoint_190_id_22.pt
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--TLM_upper_range', type=float, default=4.5)
    parser.add_argument('--TLM_lower_range', type=float, default=20.0)
    parser.add_argument('--NGRAM_upper_range', type=float, default=0.9)
    parser.add_argument('--NGRAM_lower_range', type=float, default=1.5)
    parser.add_argument('--BPE_upper_range', type=float, default=0.25)
    parser.add_argument('--BPE_lower_range', type=float, default=0.675)
    parser.add_argument('--BPE_length_penalty_upper_range', type=float, default=2.6)
    parser.add_argument('--BPE_length_penalty_lower_range', type=float, default=1.5)


    parser.add_argument('--half_precision', action='store_true')
    parser.add_argument('--only_TLM', action='store_true')

    parser.add_argument("-hyp_dev", "--hyp_dev", type=str, default='./dev_rescored.pkl')
    parser.add_argument("-hyp_test", "--hyp_test", type=str, default='./test_rescored.pkl')
...
#{'tlm_mean': tensor(-52.5750), 'tlm_std': tensor(60.1069), 'bpe_lm_weight': tensor(1.0423), 'tlm_scale': tensor(1.0404), 'ngram_scale': tensor(0.6793), 'bpe_length_penalty_weight': tensor(1.8295)
    ...
    parser.add_argument('-sclite_name', '--sclite_name', default='date', help='name of the sclite output file (without extension) will use date if blank')
    ...
    with open(args.hyp_dev, 'rb') as f:
        dev_hyps = pkl.load(f)
    with open(args.hyp_test, 'rb') as f:
        test_hyps = pkl.load(f)

    main(args, dev_hyps, test_hyps)
