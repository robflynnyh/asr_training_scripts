...
def compute_beam_ppls(args, model, tokenizer, recording_hyps): # disgusting code clean this up robert
    ...
    for i, utt in enumerate(tqdm(recording_hyps)):
        ...
        for idx in utt['beams'][0].keys():
            ...
            AM_prob = torch.tensor(cur['am_score'])
            NGRAM_prob = torch.tensor(cur['second_pass_score']) - AM_prob
            prob, cache, length_penalty = get_text_probability(args, model, tokenizer, hyptext, cached_states=kv_cache, next_target=next_target, duration_data=duration_data)
