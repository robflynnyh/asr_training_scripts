...
class transformer(nn.Module):
    ...
    def sample_idx(b_num:int, b_idx:List[torch.Tensor], negatives:int, slens:torch.Tensor, flat_idx:torch.Tensor):
        ...
        c_negative_idx = torch.randint(0, c_flat_idx.shape[0], (negatives, c_slen))
        return c_negative_idx
    ...
    def forward(self, x, length, cache=None, **kwargs):
        ...
        for i in range(self.depth):
            ...
            if i!= 0:
                ...
                z = self.checkpoint(i, self.bottleneck, z)
                z = l2norm(z, dim=-1)
                z = rearrange(z, 'b seq d -> b seq 1 d')
                scaler = self.pred_scaler
                xsimpnegs = self.checkpoint(i, self.einopsfn, z, posnegs, 'b n o d, b n k d -> b n k') * scaler
            ...
            if i != self.depth-1:
                ...
                # set padding to -1
                flat_idx[length.to('cpu').view(B,1).expand(B,N) < torch.arange(N)] = -1
                # flaten the index
                for b in range(B):
                ...
                xbottlenecked = self.checkpoint(i, self.bottleneck, x)
                ...
                posnegs = torch.cat([xp, negs], dim=2)
                # l2 normalize
                posnegs = l2norm(posnegs, dim=-1)
