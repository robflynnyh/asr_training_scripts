...
class CosineAttention(nn.Module):
    ...
    def attend(self, qkv, mask, pos_fn): 
        query, key, value = qkv
        assert query.shape == key.shape == value.shape, 'query, key, and value must have the same shape'
        
        causal_mask = torch.ones(query.shape[-2], query.shape[-2], device=query.device, dtype=torch.bool).triu(1)

        query, key = map(l2norm, (query, key))

        dots = torch.einsum('bhid,bhjd->bhij', query, key) * self.temperature
...
class transformer(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert causal, 'only causal is supported at the moment'
        ...
        self.tdnn_layer = PreNorm(dim, self.causal_conv_fn(dim))
    ...
    def causal_conv_fn(self, dim):
        self.conv1 = weight_norm(CausalConv1d(in_channels=dim, out_channels=dim, kernel_size=8, dilation=1))
        self.brn1 = nn.BatchNorm1d(dim)
        self.conv2 = weight_norm(CausalConv1d(in_channels=dim, out_channels=dim, kernel_size=5, dilation=3))
        self.brn2 = nn.BatchNorm1d(dim)
        self.conv3 = weight_norm(CausalConv1d(in_channels=dim, out_channels=dim, kernel_size=3, dilation=6))
        self.brn3 = nn.BatchNorm1d(dim)
        self.conv4 = weight_norm(CausalConv1d(in_channels=dim, out_channels=dim, kernel_size=3, dilation=9))
        self.brn4 = nn.BatchNorm1d(dim)
        self.relu = nn.ReLU()
        layers = [self.conv1, self.conv2, self.conv3, self.conv4]
        norms = [self.brn1, self.brn2, self.brn3, self.brn4]
        def causal_conv(x):
    ...
    def forward(self, x, mask=None, self_condtioning=None):
        ...
        for i, (attn, ff) in enumerate(self.layers):
            x = self.checkpoint(i, self.tdnn_layer, x) + x
