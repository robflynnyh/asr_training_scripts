...
class GumbelSigmoid():
    ...
    def __call__(self,logits):
        ...
        uniform1 = torch.rand(logits.shape, device=logits.device, dtype=logits.dtype)
        uniform2 = torch.rand(logits.shape, device=logits.device, dtype=logits.dtype)
    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, LearnableMyopicAttention(
                    dim, 
                    n_heads=heads, 
                    head_dim=dim_head, 
                    causal=causal, 
