...
def load_corpus(
    target_folder:str=request_env('EARNINGS_PATH'), 
    prefix_path=request_env('EARNINGS_BASE'),
    name=request_env('EARNINGS_NAME'), 
    transform:bool=False) -> Dict[str, CutSet]:
    ds = {}
    for split in ['train', 'dev', 'test']:
        cuts = CutSet.from_file(os.path.join(target_folder, f'{name}_cuts_{split}.json.gz'))
